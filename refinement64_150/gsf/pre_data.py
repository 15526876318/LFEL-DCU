...
def Orientation_angle1(A,A1,A2):
    R12 = A1 - A
    R13 = A2 - A
    Vz = (R12+R13)/np.linalg.norm(R12+R13) 
    Vy = np.cross(Vz,R13)/np.linalg.norm(np.cross(Vz,R13))
    Vx = np.cross(Vz,Vy)
    return Vx,Vy,Vz

def Orientation_angle2(A,A1,A2):
    R12 = A1 - A
    R13 = A2 - A
    Vz = R12/np.linalg.norm(R12) 
    Vy = np.cross(Vz,R13)/np.linalg.norm(np.cross(Vz,R13))
    Vx = np.cross(Vz,Vy)
    return Vx,Vy,Vz
...
def cal_angle_dhd(angle_arix_array,num_cs):
    angle_dhd_list = []

    for i, angle1 in enumerate(angle_arix_array):   
        for angle2 in angle_arix_array[num_cs[i]]:
            x1,y1,z1,a = angle1
            x2,y2,z2,b = angle2
            ab = b-a
            ba = -ab
            a_xy = torch.Tensor([a[0], a[1], 0]).to(device)
            b_xy = torch.Tensor([b[0], b[1], 0]).to(device)
            #az1 = Bio.PDB.Vector(a+z1)
            #bz2 = Bio.PDB.Vector(b+z2)
            #a_v = Bio.PDB.Vector(a)
            #b_v = Bio.PDB.Vector(b)
            #dhd = Bio.vectors.calc_dihedral(az1, a_v, b_v, bz2)
            dhd = mp.get_dhd(z1,ba,z2)
            theta_a = get_angle(ab,z1)
            #print(ba,z2)
            theta_b = get_angle(ba,z2)
            #print(x1.device)
            phy_a = get_angle(x1,a_xy)
            phy_b = get_angle(x2,b_xy)
            angle_dhd = theta_a, theta_b, phy_a, phy_b,dhd
            angle_dhd_list.append(angle_dhd)
    return  angle_dhd_list

def cal_angle_dhd_matrix(angle_arix_array,num_cs):
    h,w = num_cs.size()
    A = angle_arix_array
    B = angle_arix_array[num_cs]
    x1,y1,z1,a = A[:,0],A[:,1], A[:,2], A[:,3]
    x2,y2,z2,b = B[:,:,0],B[:,:,1], B[:,:,2], B[:,:,3]
    a1 = a.view(h,1,-1)
    print("a1:",a1.shape, "b:",b.shape)
    ac = a.clone()
    bc = b.clone()
    ab = b-a1
    ba = -ab
    ac[:,2] = 0
    bc[:,:,2] = 0
    a_xy = ac.repeat(1,w,1).view(-1,3)
    b_xy = bc.view(-1,3)
    print(ab.shape, z1.shape)
    ab = ab.view(-1,3)
    z1 = z1.repeat(1,w,1).view(-1,3)
    ba = ba.view(-1,3)
    z2 = z2.view(-1,3)

    x1 = x1.repeat(1,w,1).view(-1,3)
    x2 = x2.view(-1,3)
    theta_a = get_angle_matrix(ab,z1)
    theta_b = get_angle_matrix(ba,z2)
    phy_a = get_angle_matrix(x1,a_xy)
    phy_b = get_angle_matrix(x2,b_xy)
    print("single:",theta_a.shape,theta_b.shape,phy_a.shape,phy_b.shape )

    angle_dhd = torch.stack([theta_a, theta_b, phy_a, phy_b], dim =1)
    return angle_dhd
